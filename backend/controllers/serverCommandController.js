...
exports.getServerCommands = async (req, res) => {
    ...
    try {
      const server = await Server.findByPk(serverId, {
        include: [{
          model: ServerCommand,
          as: 'serverCommands', // Correct alias: matches the Server model
          include: [{
            model: Command,
            as: 'command',
...
exports.updateServerCommand = async (req, res) => {
    ...
    try {
        ...
        // Assuming you have a function to deploy commands based on premium status
        await require('../utils/deployCommands')(DISCORD_CLIENT_ID, serverId, BOT_TOKEN, req.server.premium);

        res.json({ message: 'Command status updated successfully and Discord commands deployed.', serverCommand });
    } catch (error) {
        logger.error(`Error updating server-specific command: ${error.message}`);
        res.status(500).json({ error: 'Internal server error' });
    }
};
